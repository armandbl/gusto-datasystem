...
void append_to_fits_table(const char *filename, struct s_header *fits_header, double *array) {
    ...
    long nrows;
    char extname[] = "DATA_TABLE", proctime[256], comment[256], line[4], version[] = "20251006";
    ...
    if (fits_open_file(&fptr, filename, READWRITE, &status)) {
		...
		return;
            ...
            char *ttype[] ={"MIXER", "NINT", "UNIXTIME", "NBYTES", "CORRTIME", "INTTIME", "ROW_FLAG", "Ihigh", \
			    ...
			    "Tsys", "rms", "DATA", "CHANNEL_FLAG"};

            char *tunit[] ={" ", " ", "sec", " ", " ", "sec", " ", "counts", "counts", "counts",   \
			    "counts", " ", " ", "Volts", "Volts", "Volts", "Volts", " ", " ", " ", \
		            "degrees", "degrees", "text", "Amps", "mV", "uA", "Amps", "SSB_K", "K", " ", " "};
	    ...
	    tform[27] = "1E"; //float   Tsys
	    tform[28] = "1E"; //float   rms
	    // Various indices and keywords in the per row header that depend on Band #
            if (fits_header->unit==6){ //ACS5 B1
	       tform[29] = "512E";  //32 bit float
	       tform[30] = "512I";  //16 bit int
	    }
            if (fits_header->unit==4){ //ACS3 B2
	       tform[29] = "1024E";  //32 bit float
	       tform[30] = "1024I";  //16 bit int
	    }
    ...
    if (fits_write_col(fptr, TDOUBLE, 30, nrows+1, 1, 1 * array_length, array, &status)) {
        fits_report_error(stderr, status);  // Print any error message
        return;
    }
    // Write the channel flag as a single 2*N column
    if (fits_write_col(fptr, TDOUBLE, 31, nrows+1, 1, 1 * array_length, array, &status)) {
        fits_report_error(stderr, status);  // Print any error message
        return;
    }
